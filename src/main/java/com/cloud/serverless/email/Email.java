...
//	    public Object handleRequest(SNSEvent request, Context context){
	        ...
	        dynamoDB = new DynamoDB(amazonClient);
	        // db2=new DynamoDB()
	        Table table = dynamoDB.getTable("AccountEmail");
	        ...
	        String emailMessage = emailMsgSB.toString();
	        Item item = table.getItem("emailID",userEmail);
	        System.out.println("item= "+item);
	        if (item == null) {

	            PutItemOutcome outcome  = table
	                    .putItem(new Item().withPrimaryKey("emailID", userEmail).with("emailmsg", emailMessage));
	            ...
	            for(int i=0; i<messageInfo.size(); i++){
	                context.getLogger().log(messageInfo.get(i));

	            }

	            outcome  = userEmailsTable
	                    .putItem(new Item().withPrimaryKey("emailID", userEmail).with("TimeToLive", ttl).with("Token",messageInfo.get(2)));
